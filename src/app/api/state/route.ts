...
export async function GET() {
  ...
  const [pessoas, empresas, treinamentos, equipes, equipeMembros, listasOpcoes, patrimonios, auditorias, configuracoes] = await Promise.all([
    ...
    podeAuditorias
      ? admin
          .from("auditorias")
          .select("id, standard, site_id, empresa, regional, data, status, inspetor_nome, num_colaboradores, colaboradores, respostas, modalidade, criado_por_nome, criado_em, atualizado_em, finalizado_em")
